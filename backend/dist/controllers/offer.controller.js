    ...
    constructor(_offerService) {
        ...
        this.getAllOffers = async (req, res) => {
            try {
                ...
                const { response } = await this._offerService.getAllOffers(search, page, limit);
                let status;
                if (response) {
        ...
        this.delete = async (req, res) => {
            try {
                ...
                res.status(constants_1.STATUS_CODES.INTERNAL_SERVER_ERROR).json({
                    error: error instanceof Error ? error.message : "status updation failed",
                });
            }
        };
    }
...
exports.OfferController = OfferController = __decorate([
    ...
    __param(0, (0, inversify_1.inject)(types_1.TYPES.IOfferService)),
