...
export interface IUserService{
    ...
    verifyOTP(email:string,otp:string,purpose:'signup' | 'forgot'):Promise<{response:MessageResponseDto& { user: { name: string, id: string } }}>;
    resendOTP(email:string,purpose: 'signup' | 'forgot'):Promise<{response:MessageResponseDto& { user: { name: string } }}>;
    processGoogleAuth(profile:any):Promise<{response:UserLoginResponseDto}>;
    loginUser( email:string,password:string ) : Promise<{ response : UserLoginResponseDto}>;
    forgotPassword( email:string ) : Promise<{ response: MessageResponseDto}>;
    resetPassword( email: string, password: string, confirmPassword: string ) : Promise<{ response: MessageResponseDto}>;

    fetchAllBarbers(search: string,page: number,limit: number,district: string):Promise<{ response: ListResponseDto<BarberDto>}>
    fetchBarbersAndSlotRules(page: number,limit: number,barberId: string):Promise<{ response: SlotRuleListResponseDto}>
